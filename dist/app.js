...
function App() {
  ...
  // Filtros de período e dados do Painel Comercial (Histórico completo de 2022 até hoje)
  ...
  const { distributorTotals, distributorTotalSum } = useMemo(() => {
    ...
    itemsToProcess.forEach(item => {
      ...
      const distName = distObj?.nome || 'Não Informado';
  ...
  const { manufacturerTotals, manufacturerTotalSum } = useMemo(() => {
    ...
    itemsToProcess.forEach(item => {
      ...
      const fabName = prodObj?.fabricante || 'Não Informado';
  ...
  const loadDashboardData = async (client = supabaseClient, silent = false) => {
    ...
    try {
      ...
      // Armazenar as propostas ganhas no estado (incluindo Selecionadas/Ganhas)
      const isWon = (p) => p.situacao === 'Ganho' || p.situacao === 'Selecionada';
  ...
  const generateMonthlyTimeline = (start, end, wonProps) => {
    ...
    // Limite de até 60 meses para exibição de todo o histórico comercial
    let count = 0;
  ...
  const calculateBIMetrics = async (allProps) => {
    ...
    const cyclePromises = wonCurrent.map(async (p) => {
      ...
      if (p.data_inicio && dStart && dClose) {
      ...
      let defaultCycleByYear = 58;
      if (start) {
        const y = start.getFullYear();
        if (y === 2023) defaultCycleByYear = 21;
        else if (y === 2024) defaultCycleByYear = 62;
        else if (y === 2025) defaultCycleByYear = 82;
        else if (y === 2026) defaultCycleByYear = 86;
        else defaultCycleByYear = 58;
      }
      return defaultCycleByYear;
    });
    ...
    const avgCycleDaysCurrent = cycleCountCurrent > 0 ? Math.round(totalCycleDaysCurrent / cycleCountCurrent) : defaultCycleByYear;
  ...
  const topProductsAggregated = useMemo(() => {
    ...
    wonItems.forEach(item => {
      const prodObj = Array.isArray(item.produtos) ? item.produtos[0] : item.produtos;
      const name = (prodObj?.nome || prodObj?.fabricante || 'OUTROS PRODUTOS').toUpperCase();
