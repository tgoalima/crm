...
- **Trigger `trigger_deselect_other_proposals` (achado ao vivo em 19/08, não documentado antes):** `BEFORE INSERT OR UPDATE ON propostas`, função `auto_deselect_other_proposals()` — quando uma proposta vira `situacao = 'Selecionada'`, marca as irmãs (mesmo `clickup_negocio_id`) como `situacao = 'Não selecionada'`. Provavelmente um resquício da sessão paralela: `'Não selecionada'` não é um valor que o frontend reconhece em nenhum lugar (o app.js usa `'Desconsiderada'` para o mesmo propósito, via update explícito em `handleUpdateVersionStatus`) — os dois mecanismos coexistem sem conflito hoje, mas é redundante e usa um status "fantasma" pro frontend. Não mexido ainda; considerar remover ou alinhar numa próxima revisão.
